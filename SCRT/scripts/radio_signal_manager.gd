...
func calculate_signal_quality(sig: RadioConfigParser.RadioSignal,
	...
	var freq_factor: float = 1.0 - clampf(freq_diff / sig.tolerance_freq, 0.0, 1.0)
	# 方位因子（考虑360°环绕）
	...
	# 温和的锐化曲线（从 pow 1.5 降低到 1.2，让中等匹配也有可用信号）
	var quality: float = pow(raw_signal, 1.2)
	# HF 波段衰落效果（SSTV 不叠加 HF 衰落，避免双重打击）
	if sig.band == "HF" and sig.type != "sstv":
