...
**Desktop (global commands):** help, clear (cls), status, whoami, settings (set), theme, volume (vol), reboot, exit (quit), logout, passwd, birthday, nickname, gender, users, deluser, profile, comm, mail, scan, load, vdisc, explore, dial, phonebook (pb), env (monitor), camera (cam, cctv), decode, sound, save, packages (pkg), install, uninstall, radio

**Comm subcommands:** comm (status), comm answer, comm reject, comm video, comm phonebook, comm history/log

**Disc (story loaded):** ls (dir), cd, back, open (read, cat), unlock, eject, save, clearsave, radio, fx (sfx, sound), decode, install, uninstall, packages (pkg), explore
